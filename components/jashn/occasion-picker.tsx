...
/** Portrait card ratio ~4:3. Rich animated card tile with central emblem & frosted title badge. */
function OccasionCard({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        'group relative flex flex-col justify-between overflow-hidden rounded-2xl border-2 transition-all duration-300 focus:outline-none p-2.5 sm:p-3 text-left active:scale-98 cursor-pointer',
        'w-full shadow-xs',
        active
      ...
      style={{ aspectRatio: '4/3', minHeight: '115px', background: fallback }}
      aria-pressed={active}
    ...
    >
      ...
      <div className="relative z-10 flex items-center justify-between w-full">
        <span className="px-2 py-0.5 rounded-full text-[8.5px] font-black uppercase tracking-wider bg-black/70 text-white/90 border border-white/20 backdrop-blur-md shadow-xs">
          {catText}
        </span>
        {active && (
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        {OCCASION_CATEGORIES.map((c) => (
          <button
            ...
            onClick={() => setCat(c)}
            className={cn(
              'rounded-full px-3.5 py-1.5 text-xs font-extrabold transition-all duration-200 shadow-xs active:scale-95 cursor-pointer',
              cat === c
                ? 'bg-[#7B0D1E] text-white shadow-md shadow-[#7B0D1E]/20'
