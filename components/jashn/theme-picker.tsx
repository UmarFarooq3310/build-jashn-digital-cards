      ...
      {THEMES.map((theme) => {
        ...
        return (
          <button
            ...
            className={cn(
              'group relative flex flex-col items-center gap-1 rounded-xl border p-1.5 sm:p-2 transition-all text-center',
              isSelected
          ...
          >
            ...
            {theme.isPremium ? (
              <span className={cn(
                "absolute top-1 right-1 rounded bg-amber-500/15 border border-amber-500/30 px-0.5 py-0 text-[6.5px] font-extrabold uppercase tracking-wider text-amber-600 dark:text-amber-400 leading-none",
                isUrdu && "font-urdu"
              )}>
                {t('badgePro') || 'PRO'}
              </span>
            ...
            <span
              className={cn(
                "relative flex size-6 sm:size-7 items-center justify-center rounded-lg shadow-inner transition-transform group-hover:scale-105 border",
                isLightSwatch ? "border-stone-300" : "border-black/15 dark:border-white/15"
              )}
              style={{ background: theme.previewColor }}
            >
              {isLocked ? (
                <Lock className={cn("size-2.5 drop-shadow-md", isLightSwatch ? "text-stone-800" : "text-white")} />
              ) : isSelected ? (
                <Check
                  className={cn(
                    "size-3 stroke-[3] drop-shadow-sm",
            ...
            <div className="flex flex-col items-center w-full min-w-0">
              <span className={cn("text-[10px] sm:text-[10.5px] font-bold leading-tight text-foreground truncate w-full", isUrdu && "font-urdu text-[11px]")}>
                {translatedThemeName}
              </span>
              <span className={cn("text-[8.5px] font-medium text-muted-foreground capitalize truncate w-full", isUrdu && "font-urdu text-[9px]")}>
                {translatedMotif}
              </span>
            </div>
          </button>
